...
echo "========================================"
echo "CRDT E2E Tests — $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "========================================"

# ---- TEST 1: LWW fast path (backward compat) ----
echo ""
echo "TEST 1: LWW fast path (no clock field)"
resp=$(post "$TOKEN_A" '{"content":"LWW compat: Agent A","key":"lww-compat","tags":["test"]}')
...
resp=$(post "$TOKEN_B" '{"content":"LWW compat: Agent B overwrites","key":"lww-compat","tags":["test"]}')
http=$(extract_http "$resp")
body=$(extract_body "$resp")
...
check "source=agent-b" "$src1b" "agent-b"
echo "TEST 1: PASS"
PASS=$((PASS+1))
...
resp=$(post "$TOKEN_A" '{"content":"CRDT: Agent A initial","key":"crdt-dominate","tags":["crdt"],"clock":{"agent-a":1}}')
http=$(extract_http "$resp")
body=$(extract_body "$resp")
check "HTTP 201" "$http" "201"
...
resp=$(post "$TOKEN_B" '{"content":"CRDT: Agent B dominates","key":"crdt-dominate","tags":["crdt"],"clock":{"agent-a":1,"agent-b":1}}')
http=$(extract_http "$resp")
body=$(extract_body "$resp")
...
resp=$(post "$TOKEN_A" '{"content":"CRDT: Agent A stale write","key":"crdt-dominate","tags":["crdt"],"clock":{"agent-a":1}}')
http=$(extract_http "$resp")
body=$(extract_body "$resp")
...
resp=$(post "$TOKEN_A" '{"content":"Tiebreak: Agent A","key":"tiebreak-test","tags":["crdt"],"clock":{"agent-a":2}}')
http=$(extract_http "$resp")
body=$(extract_body "$resp")
check "HTTP 201 (first write)" "$http" "201"
id4=$(echo "$body" | jq -r '.id')

resp=$(post "$TOKEN_B" '{"content":"Tiebreak: Agent B","key":"tiebreak-test","tags":["crdt"],"clock":{"agent-b":2}}')
http=$(extract_http "$resp")
body=$(extract_body "$resp")
...
check "origin_agent=agent-a" "$origin4" "agent-a"
check "HTTP 200 (dominated)" "$http4" "200"
# NOTE: Current impl does NOT merge clocks on dominated concurrent writes.
# The existing row's clock is returned as-is. This is a known deviation from
# the proposal which says "clocks merge regardless". Logged as future fix.
...
resp=$(post "$TOKEN_A" '{"content":"Unique tombstone content xyz789","key":"delete-test","tags":["crdt","deleteme"]}')
body=$(extract_body "$resp")
id5=$(echo "$body" | jq -r '.id')

# Delete it
resp=$(del "$TOKEN_A" "$id5")
http=$(extract_http "$resp")
check "DELETE HTTP 204" "$http" "204"

# GET should return 404
resp=$(get "$TOKEN_A" "$id5")
http=$(extract_http "$resp")
...
resp=$(post "$TOKEN_A" '{"content":"Revival original","key":"revival-test","tags":["crdt"],"clock":{"agent-a":1}}')
body=$(extract_body "$resp")
id6=$(echo "$body" | jq -r '.id')

# Delete it
del "$TOKEN_A" "$id6" > /dev/null

# Verify deleted
resp=$(get "$TOKEN_A" "$id6")
http=$(extract_http "$resp")
check "deleted = 404" "$http" "404"

# Revive with dominating clock
resp=$(post "$TOKEN_B" '{"content":"Revival: Agent B resurrects","key":"revival-test","tags":["crdt","revived"],"clock":{"agent-a":3,"agent-b":1}}')
http=$(extract_http "$resp")
body=$(extract_body "$resp")
...
resp=$(post "$TOKEN_A" "{\"content\":\"Idempotent write\",\"key\":\"idempotent-test\",\"tags\":[\"crdt\"],\"clock\":{\"agent-a\":1},\"write_id\":\"$WRITE_ID\"}")
http=$(extract_http "$resp")
body=$(extract_body "$resp")
